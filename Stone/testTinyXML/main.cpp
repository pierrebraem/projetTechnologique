...
//Initialisation de la librairie tinyxml2
#include "tinyxml2.h"
...
//Charge un fichier XML, retourne 0 si il n'y a pas d'erreur
int ChargerFichierXML1()
{
  tinyxml2::XMLDocument doc;
  doc.LoadFile("xml/books.xml");
  return doc.ErrorID();
}

//Récupère le titre du premier livre
const char* RecupTitreFirst()
{
  tinyxml2::XMLDocument doc;
  doc.LoadFile("xml/books.xml");

  tinyxml2::XMLNode* root = doc.FirstChildElement("Catalog");

  tinyxml2::XMLElement* book = root->FirstChildElement("Book");

  tinyxml2::XMLElement* titre = book->FirstChildElement("Title");

  return titre->GetText();
}

//Récupère le titre d'un livre à partir de son id
const char* RecupTitreID(const char* id)
{
  tinyxml2::XMLDocument doc;
  doc.LoadFile("xml/books.xml");
  ...
  tinyxml2::XMLElement* book = root->FirstChildElement("Book");
  book->QueryStringAttribute("id", id);

  tinyxml2::XMLElement* titre = book->FirstChildElement("Titre");
...
void menu()
{
  std::cout << std::endl;
  selection = 0;
  std::cout << " 3. Récupérer le titre d'un livre à partir de son id. \n";
  std::cout << " 2. Récupérer le titre du premier livre. \n";
  std::cout << " 1. Charger un fichier xml. \n";
  std::cout << " 0. Exit.\n";
  std::cout << "\t";
  std::cout << "Commande : ";
  std::cin >> selection;
  ...
  { // Charge un fichier xml
    char cmdFormat2[99];
    int erreur = ChargerFichierXML1();

    if(erreur == 0){
      strcpy(cmdFormat2, "Le fichier XML a été chargé avec succès");
    }
    else{
      strcpy(cmdFormat2, "Une erreur s'est produite lors du chargement du fichier XML");
    }
  ...
  if (selection == 2)
  { // Récupèrer le titre du premier livre
    char cmdFormat2[99];
    int erreur = ChargerFichierXML1();

    if(erreur == 0){
      const char* recupTitre = RecupTitreFirst();
      strcpy(cmdFormat2, recupTitre);
    }
    else{
      strcpy(cmdFormat2, "Une erreur s'est produite lors du changement du fichier XML");
    }
  ...
  { // Récupère le titre d'un livre à partir de son id
    /* std::cout << std::endl;
    const char* id = "";
    std::cout << "Entrer l'id du livre que vous souhaitez: \n";
    std::cout << "\t";
    std::cin >> id;
    ...
    if(erreur == 0){
      const char* recupTitre = RecupTitreID(id);
      strcpy(cmdFormat2, recupTitre);
    }
    else{
      strcpy(cmdFormat2, "Une erreur s'est produite lors du changement du fichier XML");
